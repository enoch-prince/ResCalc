...
class TextInput:
    ...
    def __init__(self, font_family="",
        ...
        self.cursor_y_pos = 0  # position where the text is blitted to the surface
    ...
    def update(self, events):
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_BACKSPACE:  # Delete at beginning of line?
        ...
        if self.cursor_visible:
            ...
            self.surface.blit(self.cursor_surface, (cursor_x_pos, self.cursor_y_pos))
    ...
    def get_text_rect(self, x=None, y=None):
        if x and y is not None:
            return self.surface.get_rect(top=x, left=y)
        else:
    ...
    def get_cursor_position(self):
        return self.cursor_position, self.cursor_y_pos
    ...
    def set_cursor_y_pos(self, value):
        self.cursor_y_pos = value


class TextOutput(TextInput):
    """
        This class should be able to output whatever information it is given
        It inherits from the TextInput class.
    """
    def __init__(self, **kwargs):
        super(TextOutput, self).__init__(**kwargs)
        self.clicked = False
        #self.surface = pygame.Surface((len(self.input_string), 30))

    def update(self):
        # Rerender text surface:
        self.surface = self.font_object.render(self.input_string, self.antialias, self.text_color)

    def set_output_string(self, text):
